...
- Hosts: an Apple Silicon Mac (arm64, 32GB/10 cores, macOS 26.5) today, an Ubuntu
  laptop (amd64) later, and possibly an Intel reviewer machine. Portability is a
  requirement.
...
- Targeting: `role` grain (`builder`/`controller`/`compute`) in minion config. top.sls
  ...
  (single-OS lab, deliberate and defensible).
- States: `podman/` (shared: pkg + config. Controller adds Quadlet
  ...
  Hard `require` from slurm states, since 26.05 demoted munge to a weak dep),
  ...
  A wrong value silently drains the node), `mariadb/` (pkg + service + idempotent SQL:
  ...
  /var/lib/rancher/k3s/agent/images/, since K3s ≥1.31.5 auto-imports at runtime),
