...
# package creates it correctly only if the slurm account already existed at
# install time, so ownership is asserted rather than assumed.
/var/spool/slurmctld:
  file.directory:
    - user: slurm
    - group: slurm
...
# directory is required too, since slurmdbd runs as the slurm user and dies in
# log_init if it cannot open its own log file.
slurmdbd:
  service.running:
    - enable: True
    - require:
...
# slurmctld registers the cluster with slurmdbd on its own, but asynchronously,
# and sacct is unusable until it has. Registering by hand turns a race into a
# state that either passed or failed.
#
# The retry covers the same race the other way: slurmdbd accepts connections a
# beat after systemd calls it started, and the first sacctmgr can land in that
# gap.
