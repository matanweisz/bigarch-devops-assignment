# Builder-only. Wraps scripts/build.sh, run straight out of the rsynced repo:
# file_roots already points there, so copying it into salt:// would just be a
...
build-artifacts:
  cmd.run:
    ...
    # --check compares both stamps, so the skip logic lives in one place and a
